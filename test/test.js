...
import { ADWebAuthAuthenticator, ActiveDirectoryAuthenticator, PlainTextAuthenticator } from '../index.js';
import { activeDirectoryConfig, activeDirectoryPassword, activeDirectoryUserName, activeDirectoryUserNameInvalid, adWebAuthConfig, plainTextConfig } from './config.js';
...
describe('PlainTextAuthenticator', () => {
    describe('With Alternative Authenticator', () => {
        const authenticator = new PlainTextAuthenticator(plainTextConfig, new ADWebAuthAuthenticator(adWebAuthConfig));
        it('Returns "true" for valid plain text credentials', async () => {
            assert.ok(await authenticator.authenticate(Object.keys(plainTextConfig)[0], Object.values(plainTextConfig)[0]));
        });
        it('Returns "false" for invalid plain text credentials', async () => {
            assert.ok(!(await authenticator.authenticate(Object.keys(plainTextConfig)[0], `${Object.values(plainTextConfig)[0]}x`)));
        });
